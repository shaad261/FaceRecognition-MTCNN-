...
def name_exists_in_csv(attendance_path, name_to_find):
    a=pd.read_csv(attendance_path)
    for i in a["NAME"]:
        if i==name_to_find :
            return True
        else:
...
class FacePredictor():
    ...
    def detectFace(self):
        ...
        while True:
                ...
                if len(bboxes) != 0:
                    ...
                    for bboxe in bboxes:
                        ...
                        if cos_similarity < cosine_threshold and proba > proba_threshold:
                            ...
                            exist=os.path.isfile("Attendance/Attendance_.csv")
            ...
            if k==ord('0'):
                if exist:
                    if name_exists_in_csv("Attendance/Attendance_.csv",str(name)):
                        engine.say("Attendance already exists..")  
                        engine.runAndWait() 
                        time.sleep(1)
                        break
                    else:
                        with open("Attendance/Attendance_.csv", "+a") as csvfile:
